...
UKF::UKF() {
  ...
  //create vector for weights and set them
  weights_ = VectorXd(2 * n_aug_ + 1);
  double lambda_na = lambda_ + n_aug_;
  weights_[0] = lambda_ / lambda_na;
  for (int i = 1; i < 2 * n_aug_ + 1; i++) {
    weights_[i] = 0.5 / lambda_na;
  }
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  // Compute the time elapsed between the previous and current measurements
  double dt = (meas_package.timestamp_ - time_us_) / 1000000.0; // dt - expressed in seconds
  time_us_ = meas_package.timestamp_;

  Prediction(dt);
  cout << "Predicted x and P" << endl << x_ << endl << P_ << endl;
  ...
  cout << "Updated x and P" << endl << x_ << endl << P_ << endl;
}
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  cout << "Before assignment predicted x" << endl << x << endl;
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  cout << "Predicted Radar Measurements" << endl << z_pred << endl << S << endl;
  ...
  //calculate Kalman gain K;
  MatrixXd K = Tc * S.inverse();
  cout << "Radar Kalman gain " << endl << K << endl;
...
void UKF::AugmentedSigmaPoints(MatrixXd *Xsig_out) {
  ...
  for (int i = 1; i <= n_aug_; i++) {
    ...
    Xsig_aug.col(i + n_aug_) = x_aug - A.col(i - 1) * sqrt(lambda_ + n_aug_);
  }
...
//  cout << "Measurement space Zsig" << endl << Zsig << endl;
